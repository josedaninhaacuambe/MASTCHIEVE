...
async function main() {
    ...
    for (const studentId of studentIds) {
        ...
        for (let si = 0; si < sessions.length; si++) {
            ...
            if (present && si % 2 === 0) {
                ...
                const level = STUDENTS.find(s => s.firstName === student?.firstName && s.lastName === student?.lastName)?.level || 'BEGINNER';
                const baseScore = { BEGINNER: 4, ELEMENTARY: 5.5, INTERMEDIATE: 6.5, ADVANCED: 7.5, COMPETITIVE: 8.5 }[level] || 5;
                const progression = si * 0.04;
                ...
                const templates = FEEDBACK_TEMPLATES[level] || FEEDBACK_TEMPLATES.BEGINNER;
    ...
    for (const plan of trainingPlansData) {
        if (plan.studentIdx >= studentIds.length)
            continue;
    ...
    for (const studentId of studentIds) {
        for (let monthOffset = 2; monthOffset >= 0; monthOffset--) {
            ...
            if (monthOffset === 2) {
                if (rng < 0.95) {
                    status = 'PAID';
                    paidAt = new Date(targetDate.getTime() + randomBetween(-5, 10) * 24 * 60 * 60 * 1000);
                }
                else {
                    status = 'OVERDUE';
                }
                ...
                if (rng < 0.88) {
                    status = 'PAID';
                    paidAt = new Date(targetDate.getTime() + randomBetween(-3, 15) * 24 * 60 * 60 * 1000);
                }
                else if (rng < 0.95) {
                    status = 'OVERDUE';
                }
                ...
                if (rng < 0.55) {
                    status = 'PAID';
                    paidAt = new Date(targetDate.getTime() + randomBetween(-2, 5) * 24 * 60 * 60 * 1000);
                }
                else if (rng < 0.85) {
                    status = 'PENDING';
                }
